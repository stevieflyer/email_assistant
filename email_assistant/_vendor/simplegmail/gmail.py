...
from .label import (
    Label,
    INBOX, SPAM, TRASH, UNREAD, STARRED, SENT, IMPORTANT, DRAFT,
)
...
class Gmail(object):
    ...
    def from_token(
        ...
        revoke_uri: Optional[Any] = None,
    ) -> Gmail:
        creds = client.AccessTokenCredentials(access_token, user_agent=user_agent, revoke_uri=revoke_uri)
        return cls(_creds=creds)
    ...
    def service(self) -> googleapiclient.discovery.Resource:
        ...
        if self.creds.access_token_expired:
            if isinstance(self.creds, client.OAuth2Credentials):
                self.creds.refresh(Http())
            else:
                raise RuntimeError(f"Expired token: {self.creds}.")
    ...
    def get_messages(
        ...
        """
        Gets messages from your account with support for pagination.
        ...
        Args:
            ...
            attachments: accepted values are 'ignore' which completely
                ignores all attachments, 'reference' which includes attachment
                information but does not download the data, and 'download' which
                downloads the attachment data to store locally. Default 'reference'.
            include_spam_trash: whether to include messages from spam or trash.
            limit: the maximum number of messages to retrieve. Default is None, meaning no limit.
            offset: the number of messages to skip before starting to collect. Default is 0.
        ...
        try:
            response = self.service.users().messages().list(
                ...
                maxResults=limit + offset if limit else None  # 使用 limit + offset
            ).execute()
            ...
            while 'nextPageToken' in response:
                if limit and len(message_refs) >= (limit + offset):
                    break
                ...
                response = self.service.users().messages().list(
                    ...
                    maxResults=limit + offset - len(message_refs) if limit else None
                ).execute()

                message_refs.extend(response['messages'])

            message_refs = message_refs[offset:(limit + offset) if limit else None]
